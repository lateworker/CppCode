...
using namespace std;
const int N = 500000;
int n, f[N + 10], siz[N + 10];
Graph<int, N + 10> g[N + 10];
void dfs(int u) {
	static int idx[N + 10];
	int psz = 0;
	f[u] = siz[u] = 0;
	for (int v : g[u]) dfs(v);
	for (int v : g[u]) idx[++psz] = v;
	stable_sort(idx + 1, idx + 1 + psz, [&](int i, int j) { return siz[i] - f[i] > siz[j] - f[j]; } );
//	cout << "IDX: " << u << ":\n";
//	for (int i = 1; i <= psz; i++) cout << idx[i] << " \n"[i == psz];
	for (int ii = 1; ii <= psz; ii++) {
		const int& v = idx[ii];
		int dt = min(siz[v] - f[v], siz[u] - f[u]);
//		cout << u << " " << v << " " << siz[u] << " " << f[u] << " V\n";
		f[u] += f[v] + (dt << 1);
		siz[u] += siz[v];
	} ++siz[u];
}
