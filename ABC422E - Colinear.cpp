...
const int N = 500000;
intl n, x[N + 10], y[N + 10];
std::mt19937 gen(time(0));
inline int randint(int l, int r) { return gen() % (r - l + 1) + l; }
inline void getij(int& i, int& j) { int k = randint(1, n - 1); i = randint(1, k), j = randint(k + 1, n); }
inline bool check(intl A, intl B, intl C) {
	int cnt = n;
	for (int i = 1; i <= n; i++) {
		if (A * x[i] + B * y[i] + C) --cnt;
...
int main() {
	...
	for (int c = 1; c <= 114; c++) {
		...
		intl A = y[j] - y[i];
		intl B = x[i] - x[j];
		intl C = y[i] * (x[j] - x[i]) - x[i] * (y[j] - y[i]);
		if (check(A, B, C)) wtln("Yes"), wtsp(A, B, C), wtln(), exit(0);
	} wtln("No");
	return 0;
}
