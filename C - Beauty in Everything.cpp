...
void slove() {
	rd(n, s + 1);
	if (n < 4) { wtln(n); return; }
	int ans = 3;
	for (int i = 4; i <= n; i++) {
		if (s[i] == '0' && s[i - 1] == '1' && s[i - 2] == '0' && s[i - 3] == '1') { ans = 4; break; }
		if (s[i] == '1' && s[i - 1] == '0' && s[i - 2] == '1' && s[i - 3] == '0') { ans = 4; break; }
	}
	int la[2][2] = { {0, 0}, {0, 0} };
	for (int i = 1; i <= n; i++) {
		bool c = s[i] - '0';
		la[c][1] = la[c][0], la[c][0] = i;
		int len = i - min(la[0][1], la[1][1]);
		if (len >= 4) chkmax(ans, len);
	}
	wtln(ans);
}
int main() {
