...
void modifyMax(int u, intl val) {
	u += n;
	for (int i = __lg(n) + 1; i; i--) pushdown(u >> i), chkmax(st[u >> i], val);
	chkmax(st[u], val);
}
...
void slove() {
	...
	n = vec.size() - 1;
	fill(tg + 1, tg + (n << 1) + 1, 0);
	for (int i = 1; i <= n; i++) st[i + n] = d * i;
	for (int u = n - 1; u; u--) pushup(u);
	intl f = 0, ptr = 1;
	for (int ii = 1; ii <= n; ii++) {
		const int& i = vec[ii]; assert(ii == i);
		modifyMax(ii, f + (intl) d * i);
		for (; ptr <= m && qry[ptr].r == i; ++ptr) updateAdd(0, lower(qry[ptr].l), qry[ptr].v);
		chkmax(f, queryMax(lower(i - k), ii - 1) - (intl) d * i);
	}
	cout << f << "\n";
}
