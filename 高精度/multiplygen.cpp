...
#define int long long
	struct _NTT {
		static constexpr int P = 998244353, G = 3, Gi = 332748118;
		int *r; int len, l;
		_NTT(int s) { init(s); }
		~_NTT() { delete []r; }
		inline void init(int s) {
			for(len=1,l=0;len<=s;len<<=1,++l);
			r = new int[len+10];
			memset(r,0,(len+10)*sizeof(int));
			for(int i=0;i<len;++i) r[i]=(r[i>>1]>>1)|((i&1)<<(l-1));
		}
		inline void qpow(int a, int b, int &ret) { while(b) { if(b&1) ret*=a,ret%=P; a*=a,b>>=1,a%=P; } }
		inline void NTT(int *num, int t) {
			for(int i=0;i<len;++i) 
				if(i<r[i]) swap(num[i],num[r[i]]);
			for(int mid=1;mid<len;mid<<=1) {
				int Wn=1,w=1; qpow(t==1?G:Gi,(P-1)/(mid<<1),Wn);
				for(int j=0;j<len;j+=(mid<<1),w=1)
					for(int k=0,x,y;k<mid;++k,w=(w*Wn)%P)
						x=num[j+k], y=w*num[j+k+mid]%P,
						num[j+k]=(x+y)%P,
						num[j+k+mid]=(x-y+P)%P;
			}
			if(t == -1){
				int inv=1; qpow(len,P-2,inv);
				for(int i=0;i<len;++i) num[i]=num[i]*inv%P;
			}
		}
	};
	struct Int {
		int *val, typ; int len, siz; 
		Int(int s) { init(s); }
		~Int() { delete []val; }
		inline void init(int s) { typ=0,len=0,siz=s,val=new int[s+10],memset(val,0,sizeof(int)*(s+10)); }
#define _digit(x) ('0'<=x&&x<='9')
		inline void _adjust() {
			for(int i=0;i<len;++i) 
				if(val[i]>=10) val[i+1]+=val[i]/10,val[i]%=10;
			while(val[len-1]==0) --len;
		}
		inline void _input() {
			
			len=0,typ=0,val[0]=getchar();
			while(isspace(val[0])){val[0]=getchar();}
			if(val[0]=='-') typ=1,val[0]=getchar();
			while(_digit(val[len]))
				val[len++]^=48, val[len]=getchar();
			val[len] = 0;
			for(int i=0;i<len/2;++i) swap(val[i],val[len-i-1]);
			_adjust();
		}
		inline void _output() {
			if(typ) putchar('-');
			for(int i=len-1;i>=0;--i) putchar(val[i]^48);
		}
	};
	inline void Int_multiply(Int &a, Int &b, Int &c, _NTT &mul) {
		c.len=a.len+b.len;
		mul.NTT(a.val, 1), mul.NTT(b.val, 1);
		for(int i=0;i<mul.len;++i) c.val[i] = a.val[i]*b.val[i]%mul.P;
		mul.NTT(a.val, -1), mul.NTT(b.val, -1);
		if((&c)!=(&a)&&(&c)!=(&b)) mul.NTT(c.val, -1);
		c._adjust();
	}
	istream &operator>> (istream &in, Int &a) {a._input();return in;}
	ostream &operator<< (ostream &out, Int &a) {a._output();return out;}
#undef int
};
...
#define U_1e2 (unsigned short)(100)
...
//	public:
	...
	void intll::multiply_abs(const intll_t& a, const intll_t& b, intll_t& c) {
		...
		aa.reserve(a.size() << 1);
		bb.reserve(b.size() << 1);
		for (const unsigned short& now : a) aa.push_back(now % U_1e2), aa.push_back(now / U_1e2);
		for (const unsigned short& now : b) bb.push_back(now % U_1e2), bb.push_back(now / U_1e2);
		convolution(aa, bb, cc);
		
		c.clear();
		for (unsigned j : {0, 1, 2, 3}) {
			bool op = j & 1;
			intll_t now((j != 0) + (j == 3), 0), res;
			for (auto u = cc.begin(); u != cc.end(); u++, op = !op) {
				if (op) now.back() += *u % U_1e2 * U_1e2;
				else now.push_back(*u % U_1e2);
				*u /= U_1e2;
			}
			calculate_abs(c, now, res, '+');
			c.swap(res);
		}
		del_zero(c);
	}
...
string mul1(const string& x, const string& y) {
	stringstream ss;
	ss.str(x); ss >> a;
	ss.str(y); ss >> b;
	_NTT mul(a.len + b.len + 1000);
	Int_multiply(a, b, c, mul);
	ss << c;
	return ss.str();
}
