...
const int N = 200000;
int n, a[2][N + 10], las[2][N + 10];
void slove() {
	rd(n), rd(a[0] + 1, a[0] + 1 + n), rd(a[1] + 1, a[1] + 1 + n);
	a[0][n + 1] = a[1][n + 1] = 0, las[0][1] = las[1][1] = 1;
	intl ans = 0;
	for (int i = 2; i <= n + 1; i++) {
		for (int c : {0, 1}) {
			las[c][i] = i;
			if (a[c][i] > a[c][i - 1]) chkmin(las[c][i], las[c][i - 1]);
			if (a[c][i] > a[c ^ 1][i - 1]) chkmin(las[c][i], las[c ^ 1][i - 1]);
		}
		ans += i - min(las[0][i], las[1][i]) + 1;
	} wtln(ans);
}
signed main() {
	int T; rd(T);
