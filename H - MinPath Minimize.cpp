...
const int N = 200000;
int n, a[N + 10], mar[N + 10], cnt[N + 10], pfre[N + 10], inv[N + 10];
void slove() {
	rd(n), rd(a + 1, a + 1 + n);
	for (int i = 1; i < n; i++) {
		int u, v; rd(u, v);
		a[v] ? chkmax(mar[u], a[v]) : ++cnt[u];
		a[u] ? chkmax(mar[v], a[u]) : ++cnt[v];
	}
	std::vector<int> vecu;
	for (int u = 1; u <= n; u++) {
		if (a[u]) inv[a[u]] = u;
		else vecu.push_back(u);
	}
	for (int i = 1; i <= n; i++) {
		pfre[i] = pfre[i - 1];
		if (!inv[i]) ++pfre[i];
	}
	std::sort(vecu.begin(), vecu.end(), [&](int u, int v) { return mar[u] == mar[v] ? cnt[u] < cnt[v] : mar[u] < mar[v]; } );
	unsigned j = 0; int mi = n + 14;
//	wtsp("pfre:"), wtsp(pfre + 1, pfre + 1 + n), wtln();
	for (int i = 1; i <= n; i++) {
		if (inv[i]) {
			if (pfre[i - 1] >= cnt[inv[i]] && mar[inv[i]] < i) { wtln(i); return; }
		} else {
			while (j < vecu.size() && mar[vecu[j]] < i) chkmin(mi, cnt[vecu[j++]]);
			if (pfre[i - 1] >= mi) { wtln(i); return; }
		}
	}
}
int main() {
